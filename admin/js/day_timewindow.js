...
$("#days_timewindows").on("submit", ".add", function (event) {
    ...
    request.done(function (data) {
        ...
        let notification = $("#empty_notification").clone().prependTo("#notifications");
        notification.removeAttr("id");
        let notification_text = $(form).data("text");
        notification.html(notification.html().replace(/{text}/g, notification_text));

        let toast = new bootstrap.Toast(notification.get(0));
